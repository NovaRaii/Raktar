...
{
    // Logo
    $this->Image('img/warehouse-logo.jpg',10,6,30);
...
function Table($header, $data)
{
    ...
    $this->SetFillColor(170, 245, 66);
    $this->SetTextColor(255);
    $this->SetDrawColor(39, 97, 24);
    $this->SetLineWidth(.3);
    ...
    foreach($data as $row)
    {
        ...
        $this->Cell($w[3],6,number_format($row['quantity']),'LR',0,'C',$fill);
        $this->Cell($w[4],6,number_format($row['id']),'LR',0,'C',$fill);
        $this->Ln();
        $fill = !$fill;
    }
